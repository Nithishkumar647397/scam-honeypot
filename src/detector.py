...
DETECTION_PATTERNS = {
    ...
    'threat': ["blocked", "suspended", "terminated", "deactivated", "frozen", "illegal", "arrested", "police", "legal action", "band ho jayega", "block ho gaya", "arrest", "kanoon", "lock", "compromised"],
    ...
    'credential': ["otp", "pin", "cvv", "password", "card number", "account number", "aadhaar", "pan card", "share details", "otp batao", "pin batao", "identity"],
...
def detect_scam(message: str, conversation_history: Optional[List[Dict]] = None) -> Tuple[bool, float, List[str], List[str]]:
    ...
    try:
        ...
        if conversation_history: confidence += _analyze_history(conversation_history)
